...
const transformFontTo = (ext, ttfBuffer) => {
  ...
  return new Buffer.from(result.buffer);
};

const writeTransformedFonts = (loaderInterface, filename, ttfBuffer) => {
  // const writeFile = Promise.promisify(fs.writeFile);

  Object.keys(transformers).map((ext) => {
    ...
    loaderInterface.emitFile(filepath, buffer);
  });
...
const createFontFace = function createFontFace(loaderInterface, ttfBuffer) {
  const filePath = loaderInterface.resourcePath;
  const dirname = path.dirname(filePath);

  const hash = crypto.createHash('md5')
  hash.update(ttfBuffer)
  const filename = hash.digest('hex')

  return writeTransformedFonts(loaderInterface, filename, ttfBuffer).then(() => {
